...
public class MagentoEcommerce {
	...
	public void setup(@Optional("chrome") String browser) {
		// Setup WebDriver before every test case
		System.setProperty("webdriver.chrome.driver",
				"C:\\Users\\FAYEEM\\eclipse-workspace\\chromedriver_win32\\chromedriver.exe");
		ChromeOptions options = new ChromeOptions();
		options.addArguments("--remote-allow-origins=*");
		options.addArguments("--disable-gpu"); // Helps in some environments
		options.addArguments("--no-sandbox"); // Useful in CI/CD environments
		options.addArguments("--disable-dev-shm-usage"); // Helps avoid crashes
